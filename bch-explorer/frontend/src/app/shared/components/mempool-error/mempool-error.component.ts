...
export class MempoolErrorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const embeddedViewRef: EmbeddedViewRef<any> =
      this.lowBalance.createEmbeddedView({});
    embeddedViewRef.detectChanges();
    const rawHtml = embeddedViewRef.rootNodes
      .map((node) => {
    ...
    if (Object.keys(MempoolErrors).includes(this.error)) {
      this.errorContent = this.sanitizer.bypassSecurityTrustHtml(
        MempoolErrors[this.error]
      );
    } else {
