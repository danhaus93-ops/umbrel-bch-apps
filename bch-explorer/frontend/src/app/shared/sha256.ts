...
// Compact RIPEMD-160 implementation for HASH160 (SHA256 + RIPEMD160)
// Based on the original reference implementation
const RIPEMD160_K1 = [
  ...
  for (let blk = 0; blk < padded.length; blk += 64) {
    ...
    for (let j = 0; j < 80; j++) {
      let t =
        (rol32(
          (al +
            ripemd160F(j, bl, cl, dl) +
            X[RIPEMD160_R1[j]] +
            RIPEMD160_K1[Math.floor(j / 16)]) |
            0,
          RIPEMD160_S1[j]
        ) +
          el) |
      ...
      t =
        (rol32(
          (ar +
            ripemd160F(79 - j, br, cr, dr) +
            X[RIPEMD160_R2[j]] +
            RIPEMD160_K2[Math.floor(j / 16)]) |
            0,
          RIPEMD160_S2[j]
        ) +
          er) |
