...
export class BlockFeeRatesGraphComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.statsObservable$ = combineLatest([
      this.widget
        ? of(this.miningWindowPreference)
        : this.radioGroupForm
            .get('dateSpan')
            .valueChanges.pipe(
      ...
      switchMap(([timespan]) => {
        ...
        this.isLoading = true;
        return this.apiService.getHistoricalBlockFeeRates$(timespan).pipe(
          tap((response) => {
            // Group by percentile
            const seriesData = this.widget
              ? { Median: [] }
              : {
                  Min: [],
                  '10th': [],
            ...
            for (const rate of response.body) {
              ...
              if (this.widget) {
                ...
                seriesData['10th'].push([
                  timestamp,
                  rate.avgFee_10,
                  rate.avgHeight,
                ]);
                seriesData['25th'].push([
                  timestamp,
                  rate.avgFee_25,
                  rate.avgHeight,
                ]);
                ...
                seriesData['75th'].push([
                  timestamp,
                  rate.avgFee_75,
                  rate.avgHeight,
                ]);
                seriesData['90th'].push([
                  timestamp,
                  rate.avgFee_90,
                  rate.avgHeight,
                ]);
            ...
            if (this.widget) {
              ...
              const medianMa = [];
              for (
                let i = maResolution - 1;
                i < seriesData['Median'].length;
                ++i
              ) {
                let avg = 0;
                ...
                avg /= maResolution;
                medianMa.push([
                  seriesData['Median'][i][0],
                  avg,
                  seriesData['Median'][i][2],
                ]);
              }
              series.push({
                zlevel: 1,
      ...
      tooltip: {
        ...
        formatter: function (data) {
          ...
          if (['24h', '3d'].includes(this.timespan)) {
            tooltip +=
              `<small>` + $localize`At block: ${data[0].data[2]}` + `</small>`;
          } else {
            tooltip +=
              `<small>` +
              $localize`Around block: ${data[0].data[2]}` +
              `</small>`;
          }
      ...
      xAxis:
        data.series.length === 0
          ...
          : {
              name: this.widget
                ? undefined
                : formatterXAxisLabel(this.locale, this.timespan),
              nameLocation: 'middle',
              nameTextStyle: {
                padding: [10, 0, 0, 0],
              },
      ...
      legend:
        this.widget || data.series.length === 0
          ...
          : {
              ...
              data: data.legends,
              selected: JSON.parse(
                this.storageService.getValue('fee_rates_legend') || 'null'
              ) ?? {
                Min: true,
                '10th': true,
                '25th': true,
      ...
      yAxis:
        data.series.length === 0
          ...
          : {
              ...
              type: 'value',
              max: (val) =>
                this.timespan === 'all' ? Math.min(val.max, 5000) : undefined,
            },
      series: data.series,
