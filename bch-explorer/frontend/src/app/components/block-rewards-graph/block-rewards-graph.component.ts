    ...
    this.statsObservable$ = this.radioGroupForm
      ...
      .valueChanges.pipe(
        ...
        switchMap((timespan) => {
          ...
          this.isLoading = true;
          return this.apiService.getHistoricalBlockRewards$(timespan).pipe(
            tap((response) => {
              this.prepareChartOptions({
                blockRewards: response.body.map((val) => [
                  val.timestamp * 1000,
                  val.avgRewards / 100000000,
                  val.avgHeight,
                ]),
                blockRewardsFiat: response.body
                  .filter((val) => val[this.currency] > 0)
                  .map((val) => [
                    val.timestamp * 1000,
                    (val.avgRewards / 100000000) * val[this.currency],
                    val.avgHeight,
                  ]),
              });
        ...
        formatter: function (data) {
          ...
          for (const tick of data) {
            if (tick.seriesIndex === 0) {
              ...
              tooltip += `${tick.marker} ${
                tick.seriesName
              }: ${this.fiatCurrencyPipe.transform(
                tick.data[1],
                null,
                this.currency
              )}<br>`;
              ...
              },
              {
                ...
                axisLabel: {
                  ...
                  formatter: function (val) {
                    return this.fiatShortenerPipe.transform(
                      val,
                      null,
                      this.currency
                    );
