...
export class TreasuriesGraphComponent implements OnInit, OnChanges, OnDestroy {
  ...
  setupSubscription(): void {
    this.subscription = combineLatest([this.redraw$, this.walletSummaries$])
      .pipe(
  ...
  processWalletData(walletSummaries: Record<string, AddressTxSummary[]>): void {
    ...
    this.treasuries.forEach((treasury) => {
      if (
        !walletSummaries[treasury.wallet] ||
        !walletSummaries[treasury.wallet].length
      ) {
        return;
      }
      ...
      const total = this.walletStats[treasury.wallet]
        ...
        : walletSummaries[treasury.wallet].reduce(
            (acc, tx) => acc + tx.value,
            0
          );

      let runningTotal = total;
      const processedData = walletSummaries[treasury.wallet]
        .map((tx) => {
      ...
      if (this.period !== 'all') {
        ...
        this.walletData[treasury.wallet] = this.walletData[
          treasury.wallet
        ].filter((d) => d[0] >= start);

        if (
          this.walletData[treasury.wallet].length === 0 ||
          this.walletData[treasury.wallet][0][0] > start
        ) {
          ...
          this.walletData[treasury.wallet].unshift([
            start,
            startBalance,
            { placeholder: true },
          ]);
      ...
      // Add current point
      this.walletData[treasury.wallet].push([
        Date.now(),
        total,
        { current: true },
      ]);
  ...
  prepareChartOptions(): void {
    ...
    for (const treasury of this.treasuries) {
      this.seriesNameToLabel[treasury.wallet] =
        ...
        formatter: function (data) {
          ...
          const activeTreasuries: { treasury: Treasury; index: number }[] =
            this.treasuries
              ...
              .filter(
                ({ treasury }) =>
                  this.selectedWallets[treasury.wallet] &&
                  this.walletData[treasury.wallet]
              );
