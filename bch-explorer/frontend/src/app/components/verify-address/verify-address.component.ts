...
import { Utxo } from '@interfaces/backend-api.interface';
...
export class VerifyAddressComponent implements OnInit, OnDestroy {
    ...
    this.stateService.recommendedFees$
      ...
      .subscribe(
        (fees: Recommendedfees) =>
          (this.priorityFeeRate = fees?.fastestFee || 0)
      );
  ...
  addressValidator(control: any) {
    ...
    this.showFallbackFee = ['p2sh', 'v0_p2wsh', 'v1_p2tr'].includes(
      addressTypeInfo.type
    );
    ...
    this.electrsApiService
      ...
      .subscribe({
        next: (utxos: Utxo[]) => {
          ...
          this.utxosError = this.utxos.length
            ? ''
        ...
        error: (error) => {
          ...
          this.utxos = [];
          this.utxosError =
            'Unable to fetch UTXOs for this address: ' +
  ...
  private getSelectedUtxo(): Utxo | undefined {
    ...
    return this.utxos.find(
      (utxo) => utxo.txid === txid && utxo.vout === Number(voutStr)
    );
  }
    ...
    try {
      const result = createMessageSigningPsbt(
        ...
        await firstValueFrom(
          this.electrsApiService.getTransactionHex$(this.getSelectedUtxo().txid)
        )
      );
  ...
  navigateToPreview(): void {
    ...
    const fragment = new URLSearchParams({
      offline: 'true',
      tx: this.generatedPsbt,
    }).toString();
    const previewPath = this.relativeUrlPipe.transform('/tx/preview');
    const url = this.router.serializeUrl(
      this.router.createUrlTree([previewPath], { fragment: fragment })
    );
