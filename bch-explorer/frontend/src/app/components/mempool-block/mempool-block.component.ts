...
export class MempoolBlockComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.mempoolBlockTransactions$ =
      this.stateService.liveMempoolBlockTransactions$.pipe(
  ...
  getOrdinal(mempoolBlock: MempoolBlock): string {
    ...
    if (this.mempoolBlockIndex === 0) {
      ...
      this.mempoolBlockIndex === this.stateService.env.KEEP_BLOCKS_AMOUNT - 1 &&
      blocksInBlock > 1
    ) {
      return $localize`:@@mempool-block.stack.of.blocks:Stack of ${blocksInBlock}:INTERPOLATION: mempool blocks`;
    } else {
      return $localize`:@@mempool-block.block.no:Mempool block ${
        this.mempoolBlockIndex + 1
      }:INTERPOLATION:`;
    }
  }
