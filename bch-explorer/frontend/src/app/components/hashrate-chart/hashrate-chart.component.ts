...
export class HashrateChartComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.widget) {
      ...
      this.seoService.setTitle(
        $localize`:@@3510fc6daa1d975f331e3a717bdf1a34efa06dff:Hashrate & Difficulty`
      );
      this.seoService.setDescription(
        $localize`:@@meta.description.bitcoin.graphs.hashrate:See hashrate and difficulty for the Bitcoin${seoDescriptionNetwork(
          this.network
        )} network visualized over time.`
      );
    ...
    this.route.fragment.subscribe((fragment) => {
      if (
        ['1w', '1m', '3m', '6m', '1y', '2y', '3y', '4y', 'all'].indexOf(
          fragment
        ) > -1
      ) {
    ...
    this.hashrateObservable$ = combineLatest(
      ...
      tap((response: any) => {
        ...
        if (
          ...
          data.difficulty[data.difficulty.length - 1].difficulty !==
        ...
        while (hashIndex < data.hashrates.length) {
          ...
          while (
            hashIndex < data.hashrates.length &&
            diffIndex < data.difficulty.length &&
            data.hashrates[hashIndex].timestamp <=
              data.difficulty[diffIndex].time
          ) {
            diffFixed.push({
              timestamp: data.hashrates[hashIndex].timestamp,
        ...
        this.prepareChartOptions({
          hashrates: data.hashrates.map((val) => [
            val.timestamp * 1000,
            val.avgHashrate,
          ]),
          difficulty: diffFixed.map((val) => [
            val.timestamp * 1000,
            val.difficulty,
          ]),
          ...
          : [
              {
                min: (value) => {
                  ...
                  const newMin = Math.floor(
                    value.min / selectedPowerOfTen.divider / 10
                  );
              ...
              },
              {
                ...
                position: 'right',
                min: (value) => {
                  const firstYAxisMin = this.chartInstance
                    .getModel()
                    .getComponent('yAxis', 0)
                    .axis.scale.getExtent()[0];
                  if (firstYAxisMin === Infinity) {
                    return value.min;
                  }
                  const selectedPowerOfTen: any =
                    selectPowerOfTen(firstYAxisMin);
                  const newMin = Math.floor(
                ...
                max: (value) => {
                  const firstYAxisMax = this.chartInstance
                    .getModel()
                    .getComponent('yAxis', 0)
  ...
  onSaveChart() {
    ...
    download(
      ...
      `hashrate-difficulty-${this.timespan}-${Math.round(
        now.getTime() / 1000
      )}.svg`
    );
    // @ts-ignore
