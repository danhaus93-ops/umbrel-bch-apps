...
export class ClockFaceComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnInit(): void {
    if (this.stateService.isBrowser) {
      this.timeSubscription = timer(0, 250)
        .pipe(
          tap(() => {
      ...
      this.blocksSubscription = this.stateService.blocks$.subscribe(
        (blocks) => {
          this.blockTimes = blocks.map((block) => [
            block.height,
            new Date(block.timestamp * 1000),
          ]);
          this.blockTimes = this.blockTimes.sort(
  ...
  updateSegments(): void {
    const now = new Date();
    this.blockTimes = this.blockTimes.filter(
      (time) => now.getTime() - time[1].getTime() <= 3600000
    );
    ...
    const times = [['start', tail], ...this.blockTimes, ['end', now]];
  ...
  getArc(startTime, endTime, r, cx, cy): any {
    ...
    if (arcLength >= 1) {
      const path = `M ${start.x} ${start.y} A ${r} ${r} 0 ${
        arcLength > 180 ? 1 : 0
      } 1 ${end.x} ${end.y}`;
      return {
        path,
