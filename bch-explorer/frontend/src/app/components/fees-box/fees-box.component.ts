...
export class FeesBoxComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.isLoading$ = combineLatest(
      ...
      map(([socket, indicators]) => {
        return (
          socket || (indicators.mempool != null && indicators.mempool !== 100)
        );
      })
    );
  ...
  setFeeGradient() {
    ...
    let feeLevelIndex = feeLevels
      .slice()
      .reverse()
      .findIndex((feeLvl) => this.fees.minimumFee >= feeLvl);
    feeLevelIndex =
    ...
    feeLevelIndex = feeLevels
      .slice()
      .reverse()
      .findIndex((feeLvl) => this.fees.fastestFee >= feeLvl);
    feeLevelIndex =
      feeLevelIndex >= 0 ? feeLevels.length - feeLevelIndex : feeLevelIndex;
    const endColor =
      '#' +
