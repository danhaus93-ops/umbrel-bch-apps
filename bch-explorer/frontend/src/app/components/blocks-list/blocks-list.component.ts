...
export class BlocksList implements OnInit {
  ...
  blocksCount: number;
  fromHeightSubject: BehaviorSubject<number> = new BehaviorSubject(
    this.fromBlockHeight
  );
  skeletonLines: number[] = [];
  lastBlockHeight = -1;
  blocksCountInitialized$: BehaviorSubject<boolean> =
  ...
  ngOnInit(): void {
        ...
        .pipe(
          ...
          tap(([_, params]) => {
            ...
            this.page === 1
              ...
              : this.fromHeightSubject.next(
                  this.blocksCount - 1 - (this.page - 1) * 15
                );
          })
        )
        .subscribe();
    ...
    this.blocks$ = combineLatest([
      this.fromHeightSubject.pipe(
        filter(
          (fromBlockHeight) => fromBlockHeight !== this.lastBlockHeightFetched
        ),
        switchMap((fromBlockHeight) => {
          this.isLoading = true;
          this.lastBlockHeightFetched = fromBlockHeight;
          return this.apiService
            .getBlocks$(this.page === 1 ? undefined : fromBlockHeight)
            .pipe(
              tap((blocks) => {
      ...
      scan((acc, blocks) => {
        ...
        if (blocks[1] && blocks[1].length) {
          this.blocksCount =
            Math.max(this.blocksCount, blocks[1][0].height) + 1;
          if (this.isMempoolModule) {
            // @ts-ignore: Need to add an extra field for the template
            blocks[1][0].extras.pool.logo =
              `/resources/mining-pools/` +
              blocks[1][0].extras.pool.slug +
              '.svg';
          }
      ...
      switchMap((blocks) => {
        if (this.isMempoolModule && this.auditAvailable) {
          blocks.forEach((block) => {
            block.extras.feeDelta = block.extras.expectedFees
              ? (block.extras.totalFees - block.extras.expectedFees) /
                block.extras.expectedFees
              : 0;
          });
        }
        return of(blocks);
