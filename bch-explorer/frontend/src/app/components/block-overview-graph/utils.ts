...
export function colorToHex(color: Color): string {
  return [color.r, color.g, color.b]
    .map((c) => Math.max(0, Math.min(Math.round(c * 255), 255)).toString(16))
    .join('');
}
...
const defaultColors: { [key: string]: ColorPalette } = {
  fee: {
    ...
    audit: [],
    marginal: [],
    baseLevel: (tx: TxView, rate: number) =>
      feeLevels.findIndex((feeLvl) => Math.max(0, rate) < feeLvl) - 1,
  },
};
for (const key in defaultColors) {
  const base = defaultColors[key].base;
  defaultColors[key].audit = base.map((color) =>
    darken(desaturate(color, 0.3), 0.9)
  );
  defaultColors[key].marginal = base.map((color) =>
    darken(desaturate(color, 0.8), 1.1)
  );
...
for (const key in contrastColors) {
  ...
  contrastColors[key].audit = base.map((color) =>
    darken(desaturate(color, 0.3), 0.9)
  );
  contrastColors[key].marginal = base.map((color) =>
    darken(desaturate(color, 0.8), 1.1)
  );
  ...
  switch (tx.status) {
    ...
    case 'found':
      if (tx.context === 'projected') {
        return (
          colors.audit[levelIndex] ||
          colors.audit[defaultMempoolFeeColors.length - 1]
        );
      } else {
        return levelColor;
      }
...
export function ageColorFunction(
  ...
  const color =
    theme !== 'contrast' && theme !== 'bukele'
      ...
      : contrastColorFunction(tx, colors, auditColors, relativeTime);

  const ageLevel = !tx.time
    ? 0
    : 0.8 *
      Math.tanh(
        (1 / 15) * Math.log2(Math.max(1, 0.6 * (relativeTime - tx.time - 60)))
      );
  return {
    r: color.r,
    g: color.g,
