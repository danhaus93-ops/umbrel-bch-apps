...
const MAX_BCH_SUPPLY = 21000000;
const MAX_SATOSHI_SUPPLY = MAX_BCH_SUPPLY * 100_000_000;
...
export class CalculatorComponent implements OnInit {
  ...
  ngOnInit(): void {
        ...
        return this.todaySelected
          ...
          : this.apiService.getHistoricalPrice$(timestamp, currency).pipe(
              map((p: any) => {
                ...
                formatted[this.currentCurrency] = Math.max(
                  0,
                  p.prices[0][this.currentCurrency]
                );
                return formatted;
              })
            );
    ...
    combineLatest([this.price$, this.form.get('fiat').valueChanges]).subscribe(
      ([price, value]) => {
        ...
        const maxFiat = price * MAX_BCH_SUPPLY;
        ...
        if (isMaxSupply) {
          ...
          this.form
            .get('fiat')
            .setValue(this.formatFiat(value), { emitEvent: false });
        }
        let rate = parseFloat((value / price).toFixed(8));
        if (rate >= MAX_BCH_SUPPLY) {
          rate = MAX_BCH_SUPPLY;
        }
        ...
        this.form
          .get('bitcoin')
          .setValue(isMaxSupply ? MAX_BCH_SUPPLY.toString() : rate.toFixed(8), {
            emitEvent: false,
          });
    ...
    combineLatest([
      ...
      this.currentPrice = price;
      const isMaxSupply = parseFloat(value) >= MAX_BCH_SUPPLY;
      this.isMaxSupply = isMaxSupply;
      ...
      this.form
        .get('fiat')
        .setValue(this.formatFiat(rate), { emitEvent: false });
      this.form
        .get('satoshis')
        .setValue(
          Math.min(Math.round(value * 100_000_000), MAX_SATOSHI_SUPPLY),
          { emitEvent: false }
        );
    });
    ...
    combineLatest([
      ...
      const isMaxSupply = bitcoinValue >= MAX_BCH_SUPPLY;
      this.isMaxSupply = isMaxSupply;
      if (isMaxSupply) {
        bitcoinValue = MAX_BCH_SUPPLY;
      ...
      const bitcoinRate = isMaxSupply
        ? MAX_BCH_SUPPLY.toString()
        : bitcoinValue.toFixed(8);
      if (isNaN(value)) {
        return;
      }
      this.form
  ...
  transformInput(name: string): void {
    ...
    if (name === 'bitcoin' && parseFloat(sanitizedValue) >= MAX_BCH_SUPPLY) {
      sanitizedValue = MAX_BCH_SUPPLY.toString();
    }
    if (
      name === 'satoshis' &&
      parseFloat(sanitizedValue) > MAX_SATOSHI_SUPPLY
    ) {
  ...
  get todaySelected() {
    return (
      this.dateModel.day === this.todayDateModel.day &&
      this.dateModel.month === this.todayDateModel.month &&
      this.dateModel.year === this.todayDateModel.year
    );
  }
