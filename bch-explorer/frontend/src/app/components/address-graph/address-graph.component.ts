...
export class AddressGraphComponent implements OnChanges, OnDestroy {
  ...
  selected = {
    [$localize`:@@7e69426bd97a606d8ae6026762858e6e7c86a1fd:Balance`]: true,
    Fiat: false,
  };
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      ...
      this.subscription = combineLatest([
        ...
        this.addressSummary$ ||
          (this.isPubkey
            ? this.electrsApiService.getScriptHashSummary$(
                (this.address.length === 66 ? '21' : '41') + this.address + 'ac'
              )
            : this.electrsApiService.getAddressSummary$(this.address)
          ).pipe(
            catchError((e) => {
              this.error = `Failed to fetch address balance history: ${
                e?.status || ''
          ...
          switchMap(([redraw, addressSummary, conversions]) => {
            ...
            if (addressSummary) {
              ...
              return this.priceService
                .getPriceByBulk$(
                  extendedSummary.map((d) => d.time),
                  'USD'
                )
                .pipe(
                  tap((prices) => {
                    if (prices.length !== extendedSummary.length) {
                      extendedSummary = extendedSummary.map((item) => ({
                        ...item,
                        price: 0,
                      }));
                    } else {
  ...
  prepareChartOptions(summary: AddressTxSummary[]) {
    ...
    this.data = processData
      .filter(({ d }) => d.txid !== undefined)
      .map(({ time, balance, d }) => [time, balance, d]);
    this.fiatData = processData.map(({ time, fiatBalance, balance, d }) => [
      time,
      fiatBalance,
      d,
      balance,
    ...
    const maxValue = this.data.reduce(
      (acc, d) => Math.max(acc, Math.abs(d[1] ?? d.value[1])),
      0
    );
    const minValue = this.data.reduce(
      (acc, d) => Math.min(acc, Math.abs(d[1] ?? d.value[1])),
      maxValue
    );

    this.adjustedRight = this.selected['Fiat'] ? +this.right + 40 : +this.right;
    this.adjustedLeft = this.selected[
      $localize`:@@7e69426bd97a606d8ae6026762858e6e7c86a1fd:Balance`
    ]
      ? +this.left
      : +this.left - 40;
    ...
    this.chartOptions = {
      ...
      tooltip: {
        ...
        formatter: function (data) {
          ...
          const date = new Date(data[0].data[0]).toLocaleTimeString(
            this.locale,
          ...
          if (hasTx) {
            const header =
              data.length === 1
                ? `${data[0].data[2].txid.slice(
                    0,
                    6
                  )}...${data[0].data[2].txid.slice(-6)}`
          ...
          if (bchData.length && fiatData.length) {
            ...
            if (
              ...
              tooltip += `${
                hasTx
                  ? `<span style="color: ${fiatColor}">${fiatSymbol} ${formatFiat(
                      fiatVal
                    )}</span><br>`
                  : ''
              }
  ...
  onChartClick(e) {
    if (this.hoverData?.length && this.hoverData[0]?.[2]?.txid) {
      this.zone.run(() => {
        const url = this.relativeUrlPipe.transform(
          `/tx/${this.hoverData[0][2].txid}`
        );
        if (
  ...
  onTooltip(e) {
    this.hoverData = (
      e?.dataByCoordSys?.[0]?.dataByAxis?.[0]?.seriesDataIndices || []
    ).map((indices) => this.data[indices.dataIndex]);
  }
  ...
  onChartInit(ec) {
    ...
    this.chartInstance.on(
      'legendselectchanged',
      this.onLegendSelectChanged.bind(this)
    );
  }
  ...
  extendSummary(summary) {
    ...
    for (let i = 0; i < extendedSummary.length - 1; i++) {
      ...
      const hours = Math.floor(
        (extendedSummary[i].time - extendedSummary[i + 1].time) / oneHour
      );
      if (hours > 1) {
