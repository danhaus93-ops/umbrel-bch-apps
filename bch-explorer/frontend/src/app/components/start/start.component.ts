...
export class StartComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  @ViewChild('blockchainWrapper', { static: true })
  blockchainWrapper: ElementRef;
  ...
  minScrollWidth: number =
    40 + 155 * (8 + 2 * Math.ceil(window.innerWidth / 155));
  ...
  ngOnInit() {
    ...
    this.blockCounterSubscription = this.stateService.blocks$.subscribe(
      (blocks) => {
        ...
        this.dynamicBlocksAmount = Math.min(
          this.blockCount,
        ...
        if (
          this.blockCount <=
          Math.min(8, this.stateService.env.KEEP_BLOCKS_AMOUNT)
        ) {
          this.onResize();
        }
      }
    ...
    this.markBlockSubscription = this.stateService.markBlock$.subscribe(
      (mark) => {
        ...
        if (mark?.blockHeight != null) {
          ...
          if (
            this.lastMark?.mempoolBlockIndex === mark.mempoolBlockIndex ||
            (mark.txid && this.lastMark?.txid === mark.txid)
          ) {
            newMark = false;
          }
        ...
        if (blockHeight != null) {
          ...
          if (
            !this.tipIsSet ||
            (blockHeight < 0 && this.mempoolOffset == null)
          ) {
    ...
    this.stateService.blocks$.subscribe((blocks: BlockExtended[]) => {
      this.countdown = 0;
      ...
      for (const sb in specialBlocks) {
        if (
          specialBlocks[sb].networks.includes(
      ...
      for (const block of blocks) {
        if (
          specialBlocks[block.height] &&
          specialBlocks[block.height].networks.includes(
            this.stateService.network || 'mainnet'
          )
        ) {
        ...
        if (
          specialBlocks[block.height - 8] &&
          specialBlocks[block.height - 8].networks.includes(
            this.stateService.network || 'mainnet'
          )
        ) {
    ...
    this.resetScrollSubscription = this.stateService.resetScroll$.subscribe(
      (reset) => {
        if (reset) {
          this.resetScroll();
  ...
  ngAfterViewChecked(): void {
    if (
      ...
      this.blockchainContainer?.nativeElement?.scrollWidth
    ) {
      this.currentScrollWidth =
        this.blockchainContainer?.nativeElement?.scrollWidth;
  ...
  onResize(): void {
    ...
    if (firstVisibleBlock != null) {
      this.scrollToBlock(
        firstVisibleBlock,
        offset + (this.isMobile ? this.blockWidth : 0)
      );
    } else {
      this.updatePages();
    }
  ...
  animateMomentum() {
    ...
    requestAnimationFrame(() => {
      ...
      if (Math.abs(this.velocity) < 0.005) {
        ...
        const deceleration =
          Math.max(0.0025, 0.001 * this.velocity * this.velocity) *
          (this.velocity > 0 ? -1 : 1);
        const displacement =
          this.velocity * dt - 0.5 * (deceleration * dt * dt);
        const dv = deceleration * dt;
        if (
          (this.velocity < 0 && dv + this.velocity > 0) ||
          (this.velocity > 0 && dv + this.velocity < 0)
        ) {
          this.velocity = 0;
        } else {
  ...
  onScroll(e) {
    ...
    const translation = this.isMobile
      ? this.chainWidth * 0.95
      : this.chainWidth * 0.5;
    const backThreshold =
      middlePage.offset + this.pageWidth * 0.5 + translation;
  ...
  scrollToBlock(height, blockOffset = 0) {
    ...
    const left =
      viewingPage.offset - this.getConvertedScrollOffset(this.scrollLeft);
  ...
  getPageAt(index: number) {
    const height =
      this.chainTip -
      this.dynamicBlocksAmount -
      (index - 1) * this.blocksPerPage;
    return {
      offset:
        this.firstPageWidth + this.pageWidth * (index - 1 - this.pageIndex),
      height: height,
  ...
  blockInViewport(height: number): boolean {
    ...
    const firstX =
      this.pages[0].offset -
      this.getConvertedScrollOffset(this.scrollLeft) +
      translation;
