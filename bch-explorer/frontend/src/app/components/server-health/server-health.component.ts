...
export class ServerHealthComponent implements OnInit {
  ...
  repoMap = {
    ...
    hybrid: 'bchexplorer.cash',
    backend: 'bitcoin-cash-explorer',
    electrs: 'electrs',
    ssr: 'bchexplorer.cash',
  };
        ...
        for (const host of hosts) {
          ...
          for (const type of [
            ...
            if (type === 'core') {
              ...
              this.colors[host.host][type] = version
                ? this.getVersionColor(version, sortedCoreVersions)
                : '';
            } else if (type === 'os') {
              const version = this.parseOsVersion(host.hashes?.os);
              this.colors[host.host][type] = version
                ? this.getVersionColor(version, sortedOsVersions)
                : '';
            } else {
