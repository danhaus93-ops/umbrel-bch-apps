...
export class AddressTransactionsWidgetComponent
  implements OnInit, OnChanges, OnDestroy
{
  ...
  startAddressSubscription(): void {
    ...
    this.transactions$ = (
      ...
      (this.isPubkey
        ...
        catchError((e) => {
          this.error = `Failed to fetch address history: ${e?.status || ''} ${
      ...
      map((summary) => {
        return summary
          ?.filter((tx) => Math.abs(tx.value) >= 1000000)
          ?.slice(0, 6);
      }),
      switchMap((txs) => {
        return zip(
          txs.map((tx) =>
            this.priceService
              .getBlockPrice$(tx.time, txs.length < 3, this.currency)
              .pipe(
                map((price) => {
                  return {
                    ...tx,
  ...
  getAmountDigits(value: number): string {
    const decimals = Math.max(
      0,
      4 - Math.ceil(Math.log10(Math.abs(value / 100_000_000)))
    );
