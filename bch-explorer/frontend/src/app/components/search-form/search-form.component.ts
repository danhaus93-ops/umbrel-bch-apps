...
export class SearchFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.stateService.networkChanged$.subscribe((network) => {
      ...
      this.regexBlockhash = getRegex(
        'blockhash',
        (network as any) || 'mainnet'
      );
    });

    this.router.events.subscribe((e: NavigationStart) => {
      // Reset search focus when changing page
    ...
    this.stateService.searchFocus$.subscribe(() => {
      if (!this.searchInput) {
        // Try again a bit later once the view is properly initialized
    ...
    this.typeAhead$ = combineLatest([
      ...
      map((latestData) => {
        ...
        const matchesBlockHeight =
          ...
          parseInt(searchText) <= this.stateService.latestBlockHeight;
        const matchesDateTime =
          this.regexDate.test(searchText) &&
          new Date(searchText).toString() !== 'Invalid Date' &&
          new Date(searchText).getTime() <= Date.now() &&
          isNetworkBitcoin;
        const matchesUnixTimestamp =
          this.regexUnixTimestamp.test(searchText) &&
          parseInt(searchText) <= Math.floor(Date.now() / 1000) &&
          isNetworkBitcoin;
        const matchesTxId =
        ...
        return {
          ...
          addresses:
            matchesAddress &&
            addressPrefixSearchResults.length === 1 &&
            searchText === addressPrefixSearchResults[0]
              ? []
  ...
  selectedResult(result: any): void {
    if (typeof result === 'string') {
      // Result is prefixed with "token-" to ensure we are not going to the transaction (/tx/) instead
  ...
  search(result?: string): void {
    ...
    if (searchText) {
      ...
      if (
        ...
        this.regexDate.test(searchText)
          ? (timestamp = Math.floor(new Date(searchText).getTime() / 1000))
          : (timestamp = searchText);
  ...
  navigate(
    ...
    if (
      ...
      this.router.navigate(
        [this.relativeUrlPipe.transform(url, swapNetwork), searchText],
        extras
      );
      ...
      : combineLatest([
          ...
          map(([poolsResponse, activePoolsResponse]) => {
            const activePoolSlugs = new Set(
              activePoolsResponse.body.pools.map((pool) => pool.slug)
            );
