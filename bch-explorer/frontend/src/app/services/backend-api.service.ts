...
export class ElectrsApiService {
  ...
  getBlock$(hash: string): Observable<BlockExtended> {
    return this.httpClient.get<BlockExtended>(
      this.apiBaseUrl + this.apiBasePath + '/api/block/' + hash
    );
  }

  listBlocks$(height?: number): Observable<BlockExtended[]> {
    return this.httpClient.get<BlockExtended[]>(
      this.apiBaseUrl + this.apiBasePath + '/api/blocks/' + (height || '')
    );
  }
  ...
  getOutspends$(hash: string): Observable<Outspend[]> {
    return this.httpClient.get<Outspend[]>(
      this.apiBaseUrl + this.apiBasePath + '/api/tx/' + hash + '/outspends'
    );
  }
  ...
  getOutspendsBatched$(txids: string[]): Observable<Outspend[][]> {
    ...
    return this.httpClient.get<Outspend[][]>(
      this.apiBaseUrl + this.apiBasePath + '/api/txs/outspends',
      { params }
    );
  }
  ...
  getBlockHashFromHeight$(height: number): Observable<string> {
    return this.httpClient.get(
      this.apiBaseUrl + this.apiBasePath + '/api/block-height/' + height,
      { responseType: 'text' }
    );
  }
  ...
  getAddress$(address: string): Observable<Address> {
    return this.httpClient.get<Address>(
      this.apiBaseUrl + this.apiBasePath + '/api/address/' + address
    );
  }
  ...
  getScriptHash$(script: string): Observable<ScriptHash> {
    return from(calcScriptHash$(script)).pipe(
      switchMap((scriptHash) =>
        this.httpClient.get<ScriptHash>(
          this.apiBaseUrl + this.apiBasePath + '/api/scripthash/' + scriptHash
        )
      )
    );
  }
  ...
  getAddressSummary$(
    address: string,
    txid?: string
  ): Observable<AddressTxSummary[]> {
    let params = new HttpParams();
    if (txid) {
      params = params.append('after_txid', txid);
    }
    return this.httpClient.get<AddressTxSummary[]>(
      this.apiBaseUrl +
        this.apiBasePath +
        '/api/address/' +
        address +
  ...
  getAddressesSummary$(
    ...
    return this.httpClient.post<AddressTxSummary[]>(
      this.apiBaseUrl + this.apiBasePath + '/api/addresses/txs/summary',
      addresses,
      { params }
    );
  }
  ...
  getScriptHashesTransactions$(
    ...
    return from(
      Promise.all(scripts.map((script) => calcScriptHash$(script)))
    ).pipe(
      switchMap((scriptHashes) =>
        this.httpClient.post<Transaction[]>(
          this.apiBaseUrl + this.apiBasePath + '/api/scripthashes/txs',
          scriptHashes,
          { params }
        )
      )
  ...
  getScriptHashSummary$(
    ...
    return from(calcScriptHash$(script)).pipe(
      switchMap((scriptHash) =>
        this.httpClient.get<AddressTxSummary[]>(
          this.apiBaseUrl +
            this.apiBasePath +
            '/api/scripthash/' +
            scriptHash +
  ...
  getAddressUtxos$(address: string): Observable<Utxo[]> {
    return this.httpClient.get<Utxo[]>(
      this.apiBaseUrl + this.apiBasePath + '/api/address/' + address + '/utxo'
    );
  }
  ...
  getScriptHashesSummary$(
    ...
    return from(
      ...
      switchMap((scriptHashes) =>
        this.httpClient.post<AddressTxSummary[]>(
          this.apiBaseUrl + this.apiBasePath + '/api/scripthashes/txs/summary',
          scriptHashes,
          { params }
        )
      )
  ...
  getAddressesByPrefix$(prefix: string): Observable<string[]> {
    ...
    return this.httpClient.get<string[]>(
      this.apiBaseUrl + this.apiBasePath + '/api/address-prefix/' + prefix
    );
  }
}
