...
class Mining {
  ...
  // Simple convertion of interval string to seconds for Valkey cache TTL.
  private getCacheTTL(interval: string | null): number {
    switch (interval) {
      case '6m':
        return 3600; // 1 hour
      case '1y':
  ...
  public async $getHistoricalBlockFees(interval: string | null = null): Promise<any> {
    ...
    if (shouldCache) {
      try {
        ...
        logger.warn(
          `Failed to retrieve historical block fees from Valkey cache: ${e instanceof Error ? e.message : e}`
        );
      }
    }
    ...
    if (shouldCache) {
      try {
        ...
        logger.warn(`Failed to cache historical block fees in Valkey: ${e instanceof Error ? e.message : e}`);
      }
    }
  ...
  public async $getBlockFeesTimespan(from: number, to: number): Promise<number> {
    return await BlocksRepository.$getHistoricalBlockFees(this.getTimeRangeFromTimespan(from, to), null, { from, to });
  }
  ...
  public async $getHistoricalBlockRewards(interval: string | null = null): Promise<any> {
    ...
    if (shouldCache) {
      try {
        ...
        logger.warn(
          `Failed to retrieve historical block rewards from Valkey cache: ${e instanceof Error ? e.message : e}`
        );
      }
    }

    const data = await BlocksRepository.$getHistoricalBlockRewards(
      this.getTimeRange(interval),
      Common.getSqlInterval(interval)
    );
    ...
    if (shouldCache) {
      try {
        ...
        logger.warn(`Failed to cache historical block rewards in Valkey: ${e instanceof Error ? e.message : e}`);
      }
    }
  ...
  public async $getHistoricalBlockSizes(interval: string | null = null): Promise<any> {
    ...
    if (shouldCache) {
      try {
        ...
        logger.warn(
          `Failed to retrieve historical block sizes from Valkey cache: ${e instanceof Error ? e.message : e}`
        );
      }
    }

    const data = await BlocksRepository.$getHistoricalBlockSizes(
      this.getTimeRange(interval),
      Common.getSqlInterval(interval)
    );
    ...
    if (shouldCache) {
      try {
        ...
        logger.warn(`Failed to cache historical block sizes in Valkey: ${e instanceof Error ? e.message : e}`);
      }
    }
  ...
  public async $getPoolsStats(interval: string | null): Promise<object> {
    ...
    if (shouldCache) {
      try {
        ...
        logger.warn(`Failed to retrieve pools stats from Valkey cache: ${e instanceof Error ? e.message : e}`);
      }
    }
    ...
    if (shouldCache) {
      try {
        ...
        logger.warn(`Failed to cache pools stats in Valkey: ${e instanceof Error ? e.message : e}`);
      }
    }
    ...
    try {
      ...
      while (toTimestamp > genesisTimestamp && toTimestamp > oldestConsecutiveBlockTimestamp) {
        ...
        if (blockStats.blockCount <= 0) {
          logger.debug(
            `No block found between ${fromTimestamp / 1000} and ${
              toTimestamp / 1000
            }, skipping hashrate indexing for this period`,
            logger.tags.mining
          );
        } else {
        ...
        if (elapsedSeconds > 1) {
          ...
          logger.debug(
            `Getting weekly pool hashrate for ${formattedDate} | ~${weeksPerSeconds.toFixed(
              2
            )} weeks/sec | total: ~${totalIndexed}/${Math.round(
              totalWeekIndexed
      ...
      if (newlyIndexed > 0) {
        logger.info(
          `Weekly mining pools hashrates indexing completed: indexed ${newlyIndexed} weeks`,
          logger.tags.mining
        );
      } else {
        logger.debug(
      ...
      loadingIndicators.setProgress('weekly-hashrate-indexing', 100);
      logger.err(
        `Weekly mining pools hashrates indexing failed. Trying again in 10 seconds. Reason: ${
  ...
  public async $generateNetworkHashrateHistory(): Promise<void> {
    ...
    try {
      const genesisData = await this.getGenesisData();
      const genesisTimestamp = genesisData.timestamp * 1000;
      const indexedTimestamp = (await HashratesRepository.$getRawNetworkDailyHashrate(null)).map(
      ...
      while (toTimestamp > genesisTimestamp && toTimestamp > oldestConsecutiveBlockTimestamp) {
        ...
        if (elapsedSeconds > 1) {
          ...
          const formattedDate = new Date(fromTimestamp).toUTCString();
          logger.debug(
            `Getting network daily hashrate for ${formattedDate} | ~${daysPerSeconds.toFixed(
              2
            )} days/sec | total: ~${totalIndexed}/${Math.round(
              totalDayIndexed
            )} (${progress}%) | elapsed: ${runningFor} seconds`,
            logger.tags.mining
          );
      ...
      loadingIndicators.setProgress('daily-hashrate-indexing', 100);
      logger.err(
        `Daily network hashrate indexing failed. Trying again later. Reason: ${e instanceof Error ? e.message : e}`,
        logger.tags.mining
      );
      throw e;
    }
  ...
  public async $indexDifficultyAdjustments(): Promise<void> {
    ...
    for (const block of blocks) {
      ...
      if (elapsedSeconds > 5) {
        ...
        logger.debug(
          `Indexing difficulty adjustment at block #${block.height} | Progress: ${progress}%`,
          logger.tags.mining
        );
        timer = new Date().getTime() / 1000;
      }
  ...
  public async $indexCoinStatsIndex(): Promise<void> {
    ...
    while (currentBlockHeight > 0) {
      ...
      for (const block of indexedBlocks) {
        ...
        await BlocksRepository.$updateCoinStatsIndexData(
          block.hash,
          txoutset.txouts,
        ...
        if (elapsedSeconds > 5) {
          logger.info(
            `Indexing coinstatsindex data for block #${block.height}. Indexed ${totalIndexed} blocks.`,
            logger.tags.mining
          );
          timer = new Date().getTime() / 1000;
        }
  ...
  private getTimeRange(interval: string | null, scale = 1): number {
    switch (interval) {
      case '4y':
        return 43200 * scale; // 12h
      case '3y':
        return 43200 * scale; // 12h
      case '2y':
        return 28800 * scale; // 8h
      case '1y':
        return 28800 * scale; // 8h
      case '6m':
        return 10800 * scale; // 3h
      case '3m':
        return 7200 * scale; // 2h
      case '1m':
        return 1800 * scale; // 30min
      case '1w':
        return 300 * scale; // 5min
      case '3d':
        return 1 * scale;
      case '24h':
        return 1 * scale;
  ...
  private getTimeRangeFromTimespan(from: number, to: number, scale = 1): number {
    ...
    switch (true) {
      case timespan > 3600 * 24 * 365 * 4:
        return 86400 * scale; // 24h
      case timespan > 3600 * 24 * 365 * 3:
        return 43200 * scale; // 12h
      case timespan > 3600 * 24 * 365 * 2:
        return 43200 * scale; // 12h
      case timespan > 3600 * 24 * 365:
        return 28800 * scale; // 8h
      case timespan > 3600 * 24 * 30 * 6:
        return 28800 * scale; // 8h
      case timespan > 3600 * 24 * 30 * 3:
        return 10800 * scale; // 3h
      case timespan > 3600 * 24 * 30:
        return 7200 * scale; // 2h
      case timespan > 3600 * 24 * 7:
        return 1800 * scale; // 30min
      case timespan > 3600 * 24 * 3:
        return 300 * scale; // 5min
      case timespan > 3600 * 24:
        return 1 * scale;
      default:
        return 1 * scale;
    }
  ...
  private async getGenesisData(): Promise<{
    timestamp: number;
    bits: number;
    difficulty: number;
  }> {
