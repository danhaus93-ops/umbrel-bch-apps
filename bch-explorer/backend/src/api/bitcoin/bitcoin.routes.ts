  ...
  private async $postTransaction(req: Request, res: Response) {
    ...
    try {
      ...
      handleError(
        ...
        400,
        e.message && e.code
          ? 'sendrawtransaction RPC error: ' + JSON.stringify({ code: e.code })
  ...
  private async $testTransactions(req: Request, res: Response) {
    try {
      ...
      handleError(
        ...
        400,
        e.message && e.code
          ? 'testmempoolaccept RPC error: ' + JSON.stringify({ code: e.code })
  ...
  private async $submitPackage(req: Request, res: Response) {
    try {
      ...
      handleError(
        ...
        400,
        e.message && e.code
          ? 'submitpackage RPC error: ' + JSON.stringify({ code: e.code })
  ...
  private async $getPrevouts(req: Request, res: Response) {
    try {
      ...
      if (
        ...
        outpoints.some((item) => !/^[a-fA-F0-9]{64}$/.test(item.txid) || typeof item.vout !== 'number')
      ) {
          ...
          try {
            ...
            if (rawPrevout) {
              prevout = {
                ...
                scriptpubkey_asm: rawPrevout.scriptPubKey.asm
                  ? transactionUtils.convertScriptSigAsm(rawPrevout.scriptPubKey.hex)
                  : '',
                ...
                ...(rawPrevout.tokenData?.nft?.capability !== undefined && {
                  token_nft_capability: rawPrevout.tokenData.nft.capability,
                }),
                ...(rawPrevout.tokenData?.nft?.commitment !== undefined && {
