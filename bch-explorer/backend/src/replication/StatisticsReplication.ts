...
class StatisticsReplication {
  ...
  public async $sync(): Promise<void> {
    ...
    for (const interval of missingIntervals) {
      logger.debug(
        `Missing ${missingStatistics[interval].size} statistics rows in '${interval}' timespan`,
        'Replication'
      );
    }
    logger.debug(
      `Fetching ${missingIntervals.join(
        ', '
      )} statistics endpoints from trusted servers to fill ${totalMissing} rows missing in statistics`,
      'Replication'
    );
      ...
      for (const stat of syncResult.data) {
        ...
        if (missingTimes.has(time)) {
          try {
            ...
            logger.err(
              `Failed to insert statistics row at ${stat.added} (${interval}) from ${syncResult.server}. Reason: ` +
  ...
  private async $getMissingStatisticsInterval(interval: any, startTime: number): Promise<Set<number>> {
    try {
      ...
      const roundedTimesAlreadyHere: number[] = Array.from(
        new Set(rows.map((row) => this.roundToNearestStep(row.added, step)))
      );

      const missingTimes = timeSteps
        .filter((time) => !roundedTimesAlreadyHere.includes(time))
        .filter((time, i, arr) => {
          // Remove outsiders
          if (i === 0) {
